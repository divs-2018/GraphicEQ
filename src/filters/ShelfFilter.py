...
class ShelfFilter(Filter):
    ...
    def H_shelf(self, f, gain, cross_over_freq_Hz, B):
        return 1 + (gain - 1) / (1 + np.pow(np.abs(f) / cross_over_freq_Hz, B))

    def apply(self, samples, frame_rate):
        fft_samples = np.fft.fft(samples)
        freqs = np.fft.fftfreq(len(fft_samples), 1.0 / frame_rate)
        
        filtered_fft_samples = fft_samples * self.H_shelf(freqs, self.gain, self.cross_over_freq_Hz, self.horiz_scale)
